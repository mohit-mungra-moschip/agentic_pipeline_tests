...
Return fixes as JSON array. For TEST_BUG: fix the test file. For APP_BUG: fix the source file."""
        ...
        if apply_failed or not current_attempt_proposed:
            ...
            internal_failures_history.append(f"Attempt #{internal_attempt+1} failed during code application:\n{err_msg}")
            applied_logs.append(f"Iter {iteration+1} (Internal Attempt {internal_attempt+1}): {err_msg}")
            internal_attempt += 1
            continue
        ...
        if passed:
            ...
            applied_logs.append(f"Iter {iteration+1} (Internal Attempt {internal_attempt+1}): Successfully healed {len(current_attempt_proposed)} file(s).")
            break
        else:
            # Revert backups immediately
            for fp, content in backups.items():
            ...
            internal_failures_history.append(
                f"Attempt #{internal_attempt+1} code changes failed verification tests. Output:\n{test_output[:2000]}"
            )
            applied_logs.append(f"Iter {iteration+1} (Internal Attempt {internal_attempt+1}): Failed verification tests.")
            internal_attempt += 1
